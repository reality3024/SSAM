...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            netF.eval()
            netB.eval()
            mem_label, avg_ent, allnum = obtain_label(dset_loaders['test'], netF, netB, netC, args)
        ...
        if len(entincorrect) != 0:
            print(entincorrect,len(entincorrect))
            # print(path[entincorrect])
            for i in range(len(entincorrect)):
                # print(pseudo_label[indx[entincorrect[i]]])
                print(path[indx[entincorrect[i]]])
                enti = torch.sum(-outputs_test[indx[entincorrect[i]]] * torch.log(outputs_test[indx[entincorrect[i]]] + args.epsilon))
                if enti < avg_ent:
                    print("--------------------------------------")
                    print("难分样本", path[indx[entincorrect[i]]])
                    num += 1
        if iter_num == 1:
            print("预训练难分样本比率{:.2f}".format(100 * num / allnum))
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            args.out_file.flush()
            print(log_str + '\n')
            print("难分样本比率{:.2f}".format(100 * num / allnum))
            netF.train()
            netB.train()
...
def obtain_label(loader, netF, netB, netC, args):
    ...
    _, predict = torch.max(all_output, 1)
    avg_ent = torch.sum(ent) / len(all_label)
    ...
    return pred_label.astype('int'), avg_ent, len(all_label)

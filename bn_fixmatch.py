...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if args.cls_par > 0 and args.plabel:
            ...
            size = pseudo_label.shape[0]
            ent = torch.sum(-pseudo_label * torch.log(pseudo_label + args.epsilon), dim=1)
            label_index = ent.topk(int(size * 0.5), largest=False)[-1]
            unlabel_index = ent.topk(int(size * 0.5), largest=True)[-1]
            classifier_loss = nn.CrossEntropyLoss()(outputs_w[label_index], pred[label_index])
            mask = max_probs[unlabel_index].ge(args.thresh).float()
            Lu = (F.cross_entropy(outputs_s[unlabel_index], targets_u[unlabel_index],
                                  reduction='none') * mask).mean()
            # output = nn.Softmax(dim=1)(outputs_w)
            # size = output.shape[0]
            # ent = torch.sum(-output * torch.log(output + args.epsilon), dim=1)
            # indx = ent.topk(int(size * 0.8), largest=False)[-1]
            #
            # print(label_index)
            # print(unlabel_index)
            classifier_loss += Lu
        ...
        if args.intra_dense or args.inter_sep:
            ...
            for i in label_index:
                for j in label_index:
                    ...
                    dist = 0.5 * (1 - cos(features_w[i].unsqueeze(0), features_w[j].unsqueeze(0)))
        ...
        if args.intra_dense and args.inter_sep:
            losses += args.intra_par * intra_dist.squeeze()
            losses += args.inter_par * inter_dist.squeeze()
            im_loss = args.intra_par * intra_dist.squeeze() + args.inter_par * inter_dist.squeeze()
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.dset == 'VISDA-C':
                ...
                loss_str = 'bn loss: {:.2f}, classifer loss: {:.2f}, im loss: {:.2f}, mask:{:.2f}'.format(bn_loss,
                                                                                                          classifier_loss,
                                                                                                          im_loss,
                                                                                                          mask.mean())
...
def obtain_label(loader, netF, netB, netC, args):
    ...
    # batch_size = all_output.shape[0]
    # ent = torch.sum(-all_output * torch.log(all_output + args.epsilon), dim=1)
    # indx = ent.topk(int(batch_size * 0.8), largest=False)[-1]
    ...
    # label_index = np.where(pred_label == predict)
    # unlabel_index = np.where(pred_label == predict)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--intra_dense', action='store_true')
    parser.add_argument('--inter_sep', action='store_true')

    parser.add_argument('--intra_par', type=float, default=1.0)
    parser.add_argument('--inter_par', type=float, default=-0.1)

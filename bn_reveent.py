...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        entropy = loss.Entropy(nn.Softmax(dim=1)(outputs_test))
        indx = entropy.topk(int(batch_size * 0.5), largest=True)[-1]
        ...
        if args.bn:
            ...
            # bn_loss = torch.mean(ones_mask * bn_loss)
            bn_loss *= args.bn_par
            print("bn loss:",bn_loss)
            losses += bn_loss
        ...
        if args.cls_par > 0 and args.plabel:
            ...
            classifier_loss = torch.mean(classifier_loss * ones_mask)
            classifier_loss *= args.cls_par
            print("pseudo label loss", classifier_loss)

...
def cal_acc_clu(loader, netF, netB, netC, softpred):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
        for i in range(len(loader)):
    ...
    ensemble_output = softpred + all_output
    _, ensemble_predict = torch.max(ensemble_output, 1)

    _, predict = torch.max(all_output, 1)
    accuracy = torch.sum(torch.squeeze(predict).float() == all_label).item() / float(all_label.size()[0])
    ensemble_acc = torch.sum(torch.squeeze(ensemble_predict).float() == all_label).item() / float(all_label.size()[0])
    # mean_ent = torch.mean(loss.Entropy(nn.Softmax(dim=1)(all_output))).cpu().data.item()

    return accuracy * 100, ensemble_acc * 100
...
def train_target(args):
    ...
    netC_orignal = network.feat_classifier(type=args.layer, class_num=args.class_num, bottleneck_dim=args.bottleneck).cuda()
    ...
    netC_orignal.load_state_dict(torch.load(modelpath))
    # netC.eval()
    for k, v in netC_orignal.named_parameters():
    ...
    while iter_num < max_iter:
        try:
            inputs_test, _, tar_idx,*_, path = next(iter_test)
        except:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            netF.eval()
            netB.eval()
            softpred = obtain_label(dset_loaders['test'], netF, netB, netC_orignal, args).cuda()
        ...
        outputs_s = netC_orignal(netB_orignal(netF_orignal(inputs_test)))
        outputs_ts = netC_orignal(netB(netF(inputs_test)))
        ...
        if args.cls_par > 0 and args.plabel:
            ...
            # classifier_loss = KLloss(outputs_test,pred)
        ...
        losses += KLloss(outputs_test,outputs_s) + KLloss(outputs_ts,outputs_s)
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.dset == 'VISDA-C':
                ...
                # softpred = obtain_label(dset_loaders['test'], netF, netB, netC, args)
                # acc_s_te, ensemble_acc= cal_acc_clu(dset_loaders['test'], netF, netB, netC, softpred)
                # log_str = 'Task: {}, Iter:{}/{}; FC Accuracy = {:.2f}%, Ensemble Accuracy = {:.2f}%'.format(args.name, iter_num, max_iter, acc_s_te, ensemble_acc)

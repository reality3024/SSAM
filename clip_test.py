...
def update_pseudo_labels(netF, netP, target_loader, source_visual_prototypes, num_classes=79, threshold=0.9, all_features=None, all_true_labels=None):
    ...
    if all_features is None or all_true_labels is None:
    ...
    logits = 15.0 * centered_target @ centered_source.T
    print(f'new logits shape{logits.shape}')
    # 4. 語意配對：Target Centroids → Source Classes
    # source_prototypes_norm = F.normalize(source_visual_prototypes, dim=-1).cpu()
    # logits = 15.0 * (all_features @ source_prototypes_norm.T)
    ...
    if retained_count > 0:
        ...
        print(f'>>> 類別覆蓋數量 (Coverage Count): {coverage_count}/{num_classes} ({coverage_rate:.2f}%)')
        ...
        for c in range(num_classes):
            ...
            if class_count > 0:
                class_distribution[c] = class_count
        
        if class_distribution:
            counts = list(class_distribution.values())
            print(f'>>> 各類別樣本分布: 平均={np.mean(counts):.1f}, 最大={max(counts)}, 最小={min(counts)}')
            print(f'>>> 有樣本的類別數: {len(class_distribution)} / {num_classes}')
    else:
        print(f'>>> 警告: 無樣本通過信心門檻 (threshold={threshold})！')
        print(f'>>> 建議降低門檻或調整 logit_scale 參數')
        coverage_count = 0
        coverage_rate = 0.0

    # ===== 動態更新 Source Prototypes（讓 CAD 吸收 Target 的環境特徵）=====
    momentum = 0.99  # 保持 90% 的 CAD 記憶，吸收 10% 的 Target 新知識  
    centered_source_gpu = centered_source.cuda()
    # 遍歷所有類別
    for c in range(source_visual_prototypes.shape[0]):
        ...
        if class_mask.sum() > 0:
            ...
            target_class_feat = all_features[class_mask].mean(dim=0)
            target_class_feat = F.normalize(target_class_feat.cuda(), dim=-1)
            
            # EMA 更新 Prototype
            centered_source_gpu[c] = momentum * centered_source_gpu[c] + (1.0 - momentum) * target_class_feat
            
    # 更新完畢後，確保 Prototypes 依然是長度為 1 的單位向量
    source_visual_prototypes = F.normalize(centered_source_gpu, dim=-1)

    return pseudo_labels.cuda(), confidence_mask.cuda(), source_visual_prototypes.cuda(), all_features, all_true_labels
...
def train_target(args):
    ...
    print(f"{'='*60}")
    (pseudo_labels_tensor, confidence_mask, source_visual_prototypes, 
     all_features, all_true_labels) = update_pseudo_labels(
        netF, netP, dset_loaders["target"],
        source_visual_prototypes, args.class_num, threshold=0.9, 

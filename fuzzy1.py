...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            netF.eval()
            netB.eval()
            softpred = obtain_label(dset_loaders['test'], netF, netB, netC, args).cuda()
...
def obtain_label(loader, netF, netB, netC, args):
    ...
    initc = aff[indx].transpose().dot(all_fea[indx])
    initc = initc / (1e-8 + aff[indx].sum(axis=0)[:, None])
    ...
    for round in range(args.clutime):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--clutime', type=int, default=1, help="cluster center update times")

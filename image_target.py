...
def data_load(args): 
    ...
    dsets["target"] = ImageList_idx(txt_tar, transform=image_train(), root=root_path)
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if acc_s_te >= best_acc:
                best_acc = acc_s_te
                best_netF = netF.state_dict().copy()
                best_netB = netB.state_dict().copy()
                best_netC = netC.state_dict().copy()
                log_str += ' (New Best!)'
    ...
    if args.issave and best_netF is not None:   
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='VISDA-C', choices=['VISDA-C', 'office', 'office-home', 'office-caltech','domain','M58'])
    ...
    for i in range(len(names)):
        ...
        args.output_dir = osp.join(args.output, args.da, args.dset, names[args.s][0].upper()+names[args.t][0].upper() + datetime.datetime.now().strftime("%m-%d_%H:%M"))

...
def get_class_similarity(teacher, T=1):
    # Find last layer
    t_layer = list(teacher.children())[-1]
    while 'Sequential' in str(t_layer):
        t_layer = list(t_layer.children())[-1]

    t_weights = list(t_layer.parameters())[2].cuda()
    ...
    cls_sim_metrix = F.softmax(cls_sim_norm / T, dim=1)

    return cls_sim_metrix

...
def data_load(args):
    ...
    dsets["source_tr"] = ImageList(tr_txt, transform=Augmentation())
...
def train_source(args):
    ...
    while iter_num < max_iter:
        try:
            (inputs_source_w, inputs_source_s), labels_source = next(iter_source)
        except:
            iter_source = iter(dset_loaders["source_tr"])
            (inputs_source_w, inputs_source_s), labels_source = next(iter_source)

        if inputs_source_w.size(0) == 1:
            continue

        iter_num += 1
        lr_scheduler(optimizer, iter_num=iter_num, max_iter=max_iter)

        inputs_source_w, inputs_source_s, labels_source = inputs_source_w.cuda(), inputs_source_s.cuda(), labels_source.cuda()
        outputs_source_w = netC(netB(netF(inputs_source_w)))
        outputs_source_s = netC(netB(netF(inputs_source_s)))
        classifier_loss1 = CrossEntropyLabelSmooth(num_classes=args.class_num, epsilon=args.smooth)(outputs_source_w,
                                                                                                   labels_source)
        classifier_loss2 = CrossEntropyLabelSmooth(num_classes=args.class_num, epsilon=args.smooth)(outputs_source_s,
                                                                                                    labels_source)
        # x = F.log_softmax(outputs_source_s, 1)
        # y = F.softmax(outputs_source_w, 1)
        # aug_loss = F.kl_div(x, y)
        # classifier_loss += aug_loss

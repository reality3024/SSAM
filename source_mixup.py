...
def train_source(args):
    ...
    while iter_num < max_iter:
        ...
        mixed_x = lam * inputs_source + (1 - lam) * inputs_source[index, :]
        # Mixup loss.
        outputs_mix = netC(netB(netF(mixed_x)))
        criterion = nn.CrossEntropyLoss()
        loss_mix = lam * criterion(outputs_mix, labels_source) + (1 - lam) * criterion(outputs_mix, labels_source[index])
        # classifier_loss += loss_mix

...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        features_test = netB(netF(inputs_w))
        ...
        if args.cls_par > 0 and args.plabel:
            ...
            classifier_loss1 = nn.CrossEntropyLoss()(outputs_test, pred)
            x = F.log_softmax(outputs_s, 1)
            y = F.softmax(outputs_test, 1)
            aug_loss = F.kl_div(x, y)
            # classifier_loss2 = nn.CrossEntropyLoss()(outputs_s, pred)
            classifier_loss = classifier_loss1 + aug_loss

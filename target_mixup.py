...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if args.cls_par > 0 and args.plabel:
            ...
            mixed_x = lam * inputs_test + (1 - lam) * inputs_test[index, :]
            # Mixup loss.
            outputs_mix = netC(netB(netF(mixed_x)))
            classifier_loss = lam * nn.CrossEntropyLoss()(outputs_mix, pred) + (1 - lam) * nn.CrossEntropyLoss()(outputs_mix,
                                                                                  pred[index])

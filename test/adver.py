...
def train_target(args):
    ...
    for k, v in netC.named_parameters():
        if args.lr_decay2 > 0:
            param += [{'params': v, 'lr': args.lr * args.lr_decay2}]
        else:
            v.requires_grad = False

    optimizer_E = optim.SGD(param_group)
    optimizer_E = op_copy(optimizer_E)
    optimizer_C = optim.SGD(param)
    ...
    while iter_num < max_iter:
        try:
            inputs_test, _, tar_idx,*_ = next(iter_test)
        except:
        ...
        if iter_num % interval_iter == 0 and args.cls_par > 0:
            netF.eval()
            netB.eval()
            mem_label, weight = obtain_label(dset_loaders['test'], netF, netB, netC, args)
            mem_label = torch.from_numpy(mem_label).cuda()
            netF.train()
            netB.train()

        inputs_test = inputs_test.cuda()
        mask = weight[tar_idx].cuda()

        iter_num += 1
        lr_scheduler(optimizer_E, iter_num=iter_num, max_iter=max_iter)
        ...
        if args.bn:
            ...
            bn_loss = torch.mean(-mask * bn_loss)
            bn_loss *= args.bn_par
        ...
        if args.cls_par > 0 and args.plabel:
            ...
            classifier_loss = torch.mean(-mask * classifier_loss)
        ...
        if args.ent:
            ...
            # im_loss = torch.mean(mask * entropy_loss)
            # im_loss *= args.ent_par
        losses = im_loss + classifier_loss + bn_loss
        losses.backward()
        optimizer_C.step()
        optimizer_E.zero_grad()
        optimizer_C.zero_grad()
        ...
        for i in range(2):
            ...
            losses = im_loss + classifier_loss
            losses.backward()
            optimizer_E.step()
            optimizer_E.zero_grad()
            optimizer_C.zero_grad()
...
def obtain_label(loader, netF, netB, netC, args):
    ...
    cls_sim_metrix = loss.get_class_similarity(netC).cpu()
    ...
    #         v, m = torch.max(all_output[i], dim=0)
    #         x = F.log_softmax(all_output[i], 0)
    #         y = F.softmax(cls_sim_metrix[m], 0)
    #         # sim += v * (1 - F.kl_div(x, y)) # reduction='sum' 大一点
    #         # sim1 += F.kl_div(x, y) * (1 - v)  # 小一点
    #         sim.append(F.kl_div(x, y) * (1 - v))
    #     sim = torch.tensor(sim)
    #     weight = 1.0 + torch.exp(-sim)
    ...
    #         # sim += v * (1 - F.kl_div(x, y)) # reduction='sum' 大一点
    #         # sim1 += F.kl_div(x, y) * (1 - v)  # 小一点
    #         sim.append(F.kl_div(x, y) * entropy[i] * (1 - v))
    #     sim = torch.tensor(sim)
    #     weight = 1.0 + torch.exp(-sim)
    ...
    return pred_label.astype('int'), weight
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--entro', action='store_true')
    # parser.add_argument('--klmax', action='store_true', help="kl(1-max)")
    parser.add_argument('--maxkl', action='store_true', help="max(1-kl)")
    # parser.add_argument('--klentromax', action='store_true', help="kl*entropy(1-max)")

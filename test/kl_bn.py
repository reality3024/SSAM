...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        cls_sim_metrix = loss.get_class_similarity(netC)

        sim = torch.tensor(0.0).cuda()
        losses = torch.tensor(0.0).cuda()

        if args.klmax:
            output = nn.Softmax(dim=1)(outputs_test)
            for i in range(len(outputs_test)):
                v, m = torch.max(output[i], dim=0)
                x = F.log_softmax(output[i], 0)
                y = F.softmax(cls_sim_metrix[m], 0)
                sim += F.kl_div(x, y) * (1 - v)
            sim *= 100
            losses += sim
        ...
        if args.maxkl:
            ...
            for i in range(len(output)):
                ...
                x = F.log_softmax(output[i], 0)
                y = F.softmax(cls_sim_metrix[m], 0)
                sim += v * (1 - F.kl_div(x, y))
            # sim /= len(outputs_test)
            losses += sim

        if args.klentromax:
            output = nn.Softmax(dim=1)(outputs_test)
            entropy = loss.Entropy(output)
            for i in range(len(output)):
                v, m = torch.max(output[i], dim=0)
                x = F.log_softmax(output[i], 0)
                y = F.softmax(cls_sim_metrix[m], 0)
                sim += F.kl_div(x, y) * entropy[i] * (1 - v)
            sim *= 100
            losses += sim

        if args.bn or args.encoder or args.last:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            args.out_file.flush()
            print('kl loss:{:.2f}, bn loss:{:.2f}, classifier loss:{:.2f}, IM loss:{:.2f}, loss:{:.2f}'.format(sim, bn_loss, classifier_loss, im_loss, losses))
            print(log_str + '\n')
            netF.train()
            netB.train()

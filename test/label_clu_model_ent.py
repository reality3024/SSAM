...
def train_target(args):
    ...
    while iter_num < max_iter:
        try:
            (inputs_w, inputs_s), y, tar_idx = next(iter_test)
        except:
        ...
        if args.cls_par > 0 and args.plabel:
            ...
            label_index = torch.where(pred == targets_u)
            clucorrect = torch.where(pred != y)
            modelcorrect = torch.where(targets_u != y)
            unlabel_index = torch.where(pred != targets_u)
            ...
            indx = ent.topk(int(size), largest=False)[-1]
            entincorrect = torch.where(targets_u[indx] != y[indx])

            print(unlabel_index)    # 不同标签样本索引
            print(clucorrect)       # 聚类标签和真实标签错误样本索引
            print(modelcorrect)     # 分类器标签和真实标签错误样本索引
            print(entincorrect)     # 按熵从小到大排序的样本标签和真实样本标签错误样本索引
            print()
            classifier_loss += Lu
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.dset == 'VISDA-C':
                ...
                print("bn loss: {:.2f}, classifer loss: {:.2f}, im loss: {:.2f}, mask:{:.2f}".format(bn_loss,
                                                                                                     classifier_loss,
                                                                                                     im_loss,
                                                                                                     mask.mean()))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--thresh', type=int, default=0.9)
